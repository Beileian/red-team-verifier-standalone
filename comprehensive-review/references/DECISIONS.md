...
- **Trigger**: 用户要求优化 comprehensive-review skill 的三个改进点
- **Chosen**: 
  1. 快速红队模式 — 新增 `红队审查`/`快速评审` 触发词，跳过完整 Rubrics
  2. 评分锚点 — 每维度定义 6 分及格线，低于 6 = 可操作缺陷
  3. 决策三角 — 输出顶端一行 ✅/⚠️/❌ 方向性判断
- **Outcome**: ✅ accepted, v1.30.0

## [1.31.1] 2026-07-28

- **Trigger**: 对话中验证 MoA review 预设可用，发现 `/moa` + `全面评审` = 真正三模型正交审查
- **Chosen**: 在 "审查者模型选择" 段后新增 MoA 集成段，注明 /moa 快捷方式 + 降级声明规则
- **Outcome**: ✅ accepted, v1.31.1